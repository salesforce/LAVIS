...
@registry.register_model("pnp_blip_caption")
class PNPBlipCaption(BlipBase):
    """
    BLIP captioning model.

    Supported model types:
        - large_coco: fine-tuned BLIP large model on COCO caption dataset (train 2014 split).

    Usage:
        >>> from lavis.models import load_model
        >>> model = load_model("pnp_blip_caption", "large_coco_train2014")
    """

    PRETRAINED_MODEL_CONFIG_DICT = {
        "large_coco_train2014": "configs/models/pnp_blip_caption_large_coco_train2014.yaml",
    }
    ...
    def generate(
        ...
        encoder_out_sample_list = []
        for i in range(num_return_sequences):
            patch_id = torch.multinomial(gradcam, num_patch).reshape(encoder_out.size(0), -1) + 1
            ...
            encoder_out_sample_list.append(encoder_out_sample)

        stacked = torch.stack(encoder_out_sample_list, dim=1)
        image_embeds = torch.flatten(stacked, start_dim=0, end_dim=1)

        # image_atts = torch.ones(image_embeds_sample.size()[:-1], dtype=torch.long).to(image.device)
        # model_kwargs = {"encoder_hidden_states": image_embeds_sample, "encoder_attention_mask": image_atts}
