...
class Img2PromptVQA(BaseModel):
    ...
    PRETRAINED_MODEL_CONFIG_DICT = {"base": "configs/models/img2prompt-vqa/img2prompt_vqa_base.yaml",
                                    }

    def __init__(self, image_question_matching_model, image_captioning_model, question_generation_model,question_generation_tokenizer,offload_model=False):
        super().__init__()
    ...
    def forward_cap(
        ...
        while min_num_captions < num_captions:
            ...
            itm_outputs = self.image_question_matching_model.itm_rank(image_embeds, image_atts, encoder_input_ids=decoder_out) #caption filter
    ...
    def forward_qa_generation(self,samples):
        ...
        contexts_for_question_generation,answers,ans_to_cap_dict = self.answer_extraction(caption)
        inputs = self.question_generation_tokenizer(contexts_for_question_generation,padding="longest",truncation=True,max_length=2048,return_tensors="pt")
        question_size = inputs.input_ids.shape[0]
        cur_b = 0
        true_input_size = 10
        outputs_list  = []
        while cur_b < question_size:
            outputs = self.question_generation_model.generate(input_ids=inputs.input_ids[cur_b:cur_b+true_input_size], attention_mask=inputs.attention_mask[cur_b:cur_b+true_input_size], num_beams=3, max_length=30)
    ...
    def prompts_construction(
        ...
        Img2Prompt = Prompt + "Contexts:" + Context_Prompt+ "\n"+Task_Prompt + "Question:" + samples['text_input'][0] + "\nAnswer:"
        return Img2Prompt
    ...
    def from_config(cls, model_config):
        ...
        cached_file = download_cached_file(
            "https://storage.googleapis.com/sfr-vision-language-research/LAVIS/projects/img2prompt/T5_large_QG.pth", check_hash=False, progress=True
        )
