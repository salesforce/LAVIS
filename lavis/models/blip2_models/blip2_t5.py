...
class Blip2T5(Blip2Base):
    ...
    PRETRAINED_MODEL_CONFIG_DICT = {
        "pretrain_flant5xl": "configs/models/blip2_pretrain_flant5xl.yaml",
        "caption_coco_flant5xl": "configs/models/blip2_caption_flant5xl.yaml",
        "pretrain_flant5xxl": "configs/models/blip2_pretrain_flant5xxl.yaml",   
    }    
    def __init__(self, 
                 img_size = 224,
                 drop_path_rate = 0,
        ...
        self.visual_encoder, self.ln_vision = self.init_vision_encoder(img_size, drop_path_rate, use_grad_checkpoint, vit_precision)
        if freeze_vit:
    ...
    def forward(self, samples):
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):  
            ...
            input_tokens = self.t5_tokenizer(samples["text_input"], padding='longest', truncation=True, max_length=self.max_text_length, return_tensors="pt").to(image.device)  
            output_tokens = self.t5_tokenizer(samples["text_output"], padding='longest', truncation=True, max_length=self.max_text_length, return_tensors="pt").to(image.device) 
            ...
            outputs = self.t5_model(inputs_embeds=inputs_embeds,
    ...
    def generate(
        ...
        with torch.cuda.amp.autocast(enabled=(self.device!=torch.device("cpu")),dtype=torch.bfloat16):  
